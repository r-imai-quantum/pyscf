...
def get_hcore(mol):
    ...
    h1e = numpy.zeros((3,n4c,n4c), complex)
    h1e[:,:n2c,:n2c] = vn
    h1e[:,n2c:,:n2c] = t
    h1e[:,:n2c,n2c:] = t
...
def get_ovlp(mol):
    ...
    t  = mol.intor('int1e_ipkin_spinor', comp=3)
    s1e = numpy.zeros((3,n4c,n4c), complex)
    s1e[:,:n2c,:n2c] = s
    s1e[:,n2c:,n2c:] = t * (.5/c**2)
    return -s1e
...
class GradientsMixin(rhf_grad.GradientsMixin):
    ...
    def hcore_generator(self, mol):
        ...
        def hcore_deriv(atm_id):
            ...
            v = numpy.zeros((3,n4c,n4c), complex)
            v[:,:n2c,:n2c] = vn
            v[:,n2c:,n2c:] = wn * (.25/c**2)
            v[:,p0:p1]         += h1[:,p0:p1]
...
def _call_vhf1_llll(mol, dm):
    ...
    vj = numpy.zeros((3,n2c*2,n2c*2), dtype=complex)
    vk = numpy.zeros((3,n2c*2,n2c*2), dtype=complex)
