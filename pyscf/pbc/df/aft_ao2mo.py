...
#
        ...
        eri_mo = numpy.zeros((nij_pair,nlk_pair), dtype=complex)
...
#
        ...
        eri_mo = numpy.zeros((nij_pair,nkl_pair), dtype=complex)
...
def get_ao_pairs_G(mydf, kpts=numpy.zeros((2,3)), q=None, shls_slice=None,
    ...
    ao_pairs_G = numpy.empty((ngrids,(i1-i0)*(j1-j0)), dtype=complex)
    for pqkR, pqkI, p0, p1 \
...
def get_mo_pairs_G(mydf, mo_coeffs, kpts=numpy.zeros((2,3)), q=None,
    ...
    mo_pairs_G = numpy.empty((ngrids,nmoi,nmoj), dtype=complex)
    nao = cell.nao
    for pqkR, pqkI, p0, p1 \
